...
#print(ret)
prev_df = pd.DataFrame(columns=["Date", "Open", "High", "Low", "Close", "Type"])
...
for i in range(1,31):
    ...
    prev_df.loc[-1] = [date,ret[0],ret[1],ret[2],ret[3],'prediction']
...
df_tot = pd.concat([prev_df,actual_df])
print(df_tot)
sns.lineplot(data=df_tot, x="Date", y="Close", hue="Type")
plt.show()
